...
const useCreateTrip = (setCreateTrip: (status: boolean) => void, setNewTrip: (id: string) => void) => {
    ...
    const handleCreate = async () => {
        if (nameRef.current.length <= 3) {
            ...
            setErrorMessage('The maximum number of characters for the trip name is 27.');   
        } else if (!startDateRef.current || !endDateRef.current) {
            setErrorMessage('At least one date is invalid, please input a proper date');
        } else if (!color) {
